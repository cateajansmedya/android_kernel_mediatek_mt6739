...
#define SPK_WARM_UP_TIME        (25)	/* unit is ms */
	if (enable) {
		...
		AudDrv_GPIO_EXTAMP_Select(true, extamp_sound_mode);
...
static int mt6357_codec_probe(struct snd_soc_codec *codec)
{
	...
	mCodec_data = kzalloc(sizeof(mt6357_Codec_Data_Priv), GFP_KERNEL);
	if (!mCodec_data) {
		/*pr_debug("Failed to allocate private data\n");*/
		return -ENOMEM;
	}
	snd_soc_codec_set_drvdata(codec, mCodec_data);
	memset((void *)mCodec_data, 0, sizeof(mt6357_Codec_Data_Priv));
