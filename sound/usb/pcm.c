...
static int prof_in_frame_num;
static int prof_in_len;
static unsigned long prof_in_j;

static inline int prof_in_print(void)
{
	if (time_is_before_eq_jiffies(prof_in_j + (HZ * 2))) {
		prof_in_j = jiffies;
		return 1;
	}
	return 0;
...
static int start_endpoints(struct snd_usb_substream *subs, bool can_sleep)
{
	...
	if (!test_and_set_bit(SUBSTREAM_FLAG_DATA_EP_STARTED, &subs->flags)) {
		...
		dev_info(&subs->dev->dev, "Starting data EP @%p %s\n", ep,
			       ...
			       struct urb *urb)
{
	...
	for (i = 0; i < urb->number_of_packets; i++) {
		...
		profile_len = profile_len + bytes;
	}

	if (period_elapsed)
		snd_pcm_period_elapsed(subs->pcm_substream);

	if (prof_in_frame_num == 0) {
		prof_in_frame_num = current_frame_number;
		pr_notice("%s %d len=%d frn=%d frb=%d prd=%ld\n", __func__,
			urb->number_of_packets, urb->iso_frame_desc[0].actual_length,
			current_frame_number,
			runtime->frame_bits, runtime->period_size);
		prof_in_len = profile_len;
		prof_in_j = jiffies;
	} else {
		if (prof_in_print()) {
			pr_notice("%s %d len=%d %d->%d len=%d\n", __func__,
				urb->number_of_packets, urb->iso_frame_desc[0].actual_length,
				prof_in_frame_num, current_frame_number,
				prof_in_len + profile_len);
			prof_in_frame_num = current_frame_number;
			...
			prof_in_len = prof_in_len + profile_len;
		}
	}
}
					     ...
					     int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_STOP:
		...
		pr_notice("%s Stop Capture frm=%d->%d len=%d\n", __func__,
			prof_in_frame_num, usb_get_current_frame_number(subs->dev),
			prof_in_len);
