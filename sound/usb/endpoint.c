...
static unsigned int isoc_in_state;
static unsigned int isoc_in_len;
...
static void snd_complete_urb(struct urb *urb)
{
	...
	if (usb_pipeout(ep->pipe)) {
		...
		if (usb_pipeisoc(ep->pipe) && ((isoc_in_state % 1000) == 1)) {
			pr_info("[0]len=%d actl=%d ttl=%d st=%d NoP=%d\n",
				urb->iso_frame_desc[0].actual_length,
				urb->actual_length,
				isoc_in_len, (isoc_in_state-1),
		...
		if (isoc_in_state) {
			isoc_in_state++;
			isoc_in_len = isoc_in_len + urb->actual_length;
		}
...
int snd_usb_endpoint_start(struct snd_usb_endpoint *ep, bool can_sleep)
{
	...
	if (usb_pipein(ep->pipe) && usb_pipeisoc(ep->pipe))
		isoc_in_state = 1;
