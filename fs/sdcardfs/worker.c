...
struct workqueue_struct *sdcardfs_asyn_wq;
struct workqueue_struct *sdcardfs_sync_wq;
struct kmem_cache *sdcardfs_work_cachep;
...
static void get_derive_permissions_recursive_internal(struct dentry *parent)
{
	...
	list_for_each_entry(dentry, &parent->d_subdirs, d_child) {
		...
		if (inode) {
			...
			/*
			 * negative returned value means inode is evicted by others during
			 * permission handling. Stop handling this child in this case.
			 */
			if (get_derived_permission(parent, dentry) < 0) {
			...
			/*
			 * ensure dentry really has inode now, and try to not touch inode
			 * in this layer anymore after checking.
			 */
			...
			/*
			 * we assume the inode is not freed (maybe has been evicted or invalidated)
			 * at this time so we can still access inode->i_mutex here.
			 */
			mutex_unlock(&inode->i_mutex);
		}
	}
