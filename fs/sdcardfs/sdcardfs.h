...
static inline int get_mode(struct sdcardfs_inode_info *info) {
	...
	/* avoid get_mode being crash if inode was evicted before */
	lower_inode = info->lower_inode;

	if (lower_inode)
		owner_mode = lower_inode->i_mode & 0700;
	else
		owner_mode = 0;
...
extern int  get_derived_permission(struct dentry *parent, struct dentry *dentry);
extern int  get_derived_permission_new(struct dentry *parent, struct dentry *dentry, struct dentry *newdentry);
