...
unsigned int sysctl_sched_mtk_lb_enhance = 1;
...
#else
unsigned int sysctl_sched_mtk_lb_enhance;
			...
			bool *overload, bool *overutilized, bool *intra_overutil)
{
	...
	for_each_cpu_and(i, sched_group_cpus(group), env->cpus) {
		...
		if (sysctl_sched_mtk_lb_enhance)
				   ...
				   struct sg_lb_stats *sgs)
{
	...
	if (sysctl_sched_mtk_lb_enhance && sgs->sum_nr_running == 0) {
...
#ifdef CONFIG_HMP
	if (!sysctl_sched_mtk_lb_enhance &&
			sched_feat(SCHED_HMP) && cpumask_first_and(nohz.idle_cpus_mask,
