...
{
#ifndef __HCT_TP_INCELL_RST_IN_LCM__
    FTS_FUNC_ENTER();
...
{
#ifdef __HCT_FTS_POWER_NOT_SUSPENG__
    int ret;
...
static int tpd_irq_registration(struct fts_ts_data *ts_data)
{
    ...
    enable_irq_wake(ts_data->irq); //@20170707 modify by mtk for fix bug(deep sleep cannot resume) 
    return ret;
}
...
//hct-drv add for fwvno by qhs end
#if FTS_PSENSOR_EN
	wake_lock_init(&hct_tp_wake_lock, WAKE_LOCK_SUSPEND, "hct_tp_wake_lock");
#endif
    tpd_load_status = 1;
