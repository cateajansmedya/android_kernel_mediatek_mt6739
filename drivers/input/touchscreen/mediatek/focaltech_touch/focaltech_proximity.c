...
int fts_proximity_recovery(struct i2c_client *client)
{
    ...
    if (fts_proximity_data.mode)
        ret = fts_enter_proximity_mode(client, ENABLE);
...
static int ps_enable_nodata(int en)
{
    ...
    err = fts_enter_proximity_mode(fts_data->client, en);
...
static bool g_is_ps_suspend = false;
/*****************************************************************************
...
{
	if ((fts_proximity_data.mode == ENABLE) && g_is_ps_suspend)
	{
		g_is_ps_suspend = false;
		return 0;
	}
	else{
		g_is_ps_suspend = false;
