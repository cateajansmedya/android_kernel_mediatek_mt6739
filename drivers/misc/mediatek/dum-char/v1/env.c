	...
	if (is_first_read_write == 1) {
		...
		if (!strcmp(g_env.sig, ENV_SIG) && !strcmp(g_env.sig_1, ENV_SIG)) {
			...
			if (checksum != g_env.checksum) {
				pr_info("[%s]checksum mismatch s %d d %d!\n", MODULE_NAME,
				       g_env.checksum, checksum);
				env_valid = 0;
				goto end;
			} else {
				pr_info("[%s]ENV initialize success\n", MODULE_NAME);
				env_valid = 1;
			}

		} else {
			pr_info("[%s]ENV SIG Wrong\n", MODULE_NAME);
			env_valid = 0;
			goto end;
		}
			      ...
			      loff_t *ppos)
{
	...
	if (buffer == NULL) {
		ret = -ENOMEM;
		pr_info("[env_proc_write]alloc buffer fail\n");
		goto fail_malloc;
	}
	...
	if (i == size) {
		pr_info("write fail, buffer:%s\n", buffer);
		ret = -EFAULT;
		goto end;
	} else {
		pr_info("[env_proc_write]name :%s,value:%s\n", buffer, buffer + v_index);
	}
	...
	if (!strcmp(buffer, DATA_FREE_SIZE_TH_NAME)) {
		...
		if (value_new >= data_free_size) {
			pr_info("new value %llx more than data free size %llx, setting fail\n",
...
#else
		pr_info("[env_proc_write]it don't support limit sdcard size\n");
		ret = -EFAULT;
		goto end;
#endif
	} else {
...
#endif
{
	...
	pr_info("[%s]ENV initialize env_part_addr %llx\n", MODULE_NAME, env_part_addr);

#ifdef CONFIG_MTK_EMMC_SUPPORT
	if (!env_part_addr) {
		pr_info("env_part_addr is 0, env_init fail\n");
		return;
	}
	...
	if (env_proc == NULL) {
		pr_info("create /proc/lk_env fail\n");
		return;
	}
	...
	if (ret < 0) {
		pr_info("[%s]read_env_area fail, ret = %x\n", MODULE_NAME, ret);
		env_valid = 0;
		goto end;
	}
...
static void load_default_env(void)
{
	...
	pr_info("[%s]load default env\n", MODULE_NAME);

	tmp = get_env(DATA_FREE_SIZE_TH_NAME);
	if (tmp == NULL) {
		pr_info("[%s]can not find %s,set the default value\n", MODULE_NAME,
...
add:
	...
	if (len > (&env_data[CFG_ENV_DATA_SIZE] - env)) {
		pr_info("## Error: environment overflow, \"%s\" deleted\n", name);
		return -1;
	}
	while ((*env = *name++) != '\0')
		env++;
...
int ENV_Nand_Read(char *env_buf)
{
	...
	if (misc_mtd_number < 0) {
		pr_info("ENV_Nand_Read: wrong misc mtd number: %d!!!\n", misc_mtd_number);
		return -1;
	}
	mtd = get_mtd_device(NULL, misc_mtd_number);
	if (!mtd) {
		pr_info("ENV_Nand_Read: get device info fail!!!\n");
		put_mtd_device(mtd);
		return -1;
	}
	...
	if (result) {
		pr_info("ENV_Nand_Read: read env data fail, result = %d !!!\n", result);
		put_mtd_device(mtd);
		return -1;
	}
	put_mtd_device(mtd);
...
int ENV_Nand_Write(char *env_buf)
{
	...
	if (result) {
		pr_info("ENV_Nand_Write: erase device failed");
		put_mtd_device(mtd);
		return -1;
	}
	...
	if (result) {
		pr_info("ENV_Nand_Write: Write env data fail, result = %d !!!\n", result);
		put_mtd_device(mtd);
		return -1;
	}
