...
{
	if (!cmd) {
		pr_info("DumChar ERROR:no space for msdc_ioctl\n");
		return -EINVAL;
	}
	if (addr < 0) {
		pr_info("DumChar ERROR:Wrong Address %llx for emmc!\n", addr);
		return -EINVAL;
	}

	if (totalsize > MAX_SD_BUFFER) {
		pr_info("DumChar ERROR:too mucn bytes for msdc\n");
		return -EINVAL;
	}
	...
	if (addr % 512 == 0)
		...
		pr_info("DumChar ERROR: Wrong Address\n");
		return -EINVAL;
	}
...
*	pr_info("cmd->opcode=%d MSDC_SINGLE_READ_WRITE =(2) MSDC_MULTIPLE_READ_WRITE =(3)\n",cmd->opcode);
*	pr_info("cmd->host_num=%d supose=1\n",cmd->host_num);
*	pr_info("cmd->iswrite=%d write=1 read=0\n",cmd->iswrite);
*	pr_info("cmd->trans_type=%d\n",cmd->trans_type);
*	pr_info("cmd->total_size=%d\n",cmd->total_size);
*	pr_info("cmd->address=%d\n",cmd->address);
*	pr_info("cmd->buffer=%p\n",cmd->buffer);
*	pr_info("cmd->cmd_driving=%d\n",cmd->cmd_driving);
*	pr_info("cmd->dat_driving=%d\n",cmd->dat_driving);
*	pr_info("cmd->clock_freq=%d\n",cmd->clock_freq);
*	pr_info("cmd->result=%d\n",cmd->result);
*	pr_info("***************************\n");
*/
	return 0;
...
void emmc_create_sys_symlink(struct mmc_card *card)
{
	...
	if (!mmc_card_mmc(card)) {
		pr_info("mtk-msdc %s: NOT emmc card,\n", __func__);
		return;
	}
			      ...
			      Region part)
{
	...
	if (init_sd_cmd(&cmd, addr, (u32 *) buf, 0, 0, count, 0, part)) {
		pr_info("DumChar:init sd_cmd fail\n");
		return -EINVAL;
	}
	...
	if (addr % ALIE_LEN != 0 || (addr + count) % ALIE_LEN != 0) {
		...
		while (buflen > 0) {
			...
			if (retlen > 0) {
				...
				buflen -= retlen;
				pbuf2 += retlen;
				pr_info("while retlen > 0 total_retlen=%d\n", (int)total_retlen);
			} else
				break;
		}
		set_fs(curr_fs);
		...
		if (total_retlen == (endaddr - startaddr)) {
			...
			if (n != 0)
				pr_info("read fail in DumChar_sd_read\n");

			total_retlen = count - n;
		} else
			pr_info("read fail DumChar_sd_read!\n");
...
ssize_t dumchar_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (fo->act_filp == (struct file *)0xffffffff) {
		...
		pr_info("[dumchar_read] show_stack*************************************\n");
		show_stack(NULL, NULL);
		return -EINVAL;
	}

	if (dev->type != NAND && dev->type != EMMC) {
		pr_info("DumChar:Wrong Dummy device Type %d ,it should be MTD or SDCARD!\n",
		       dev->type);
		return -EINVAL;
	}
	...
	if (dev->type == EMMC) {
		...
		case BOOT_1:
			...
			break;
		default:
			pr_info("DumChar: Wrong EMMC Region\n");
			return -EINVAL;
		}
...
ssize_t dumchar_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (fo->act_filp == (struct file *)0xffffffff) {
		pr_info
		    ("[%s] Forbidded to access %s with dumchar(/dev/%s), the %s partition is managed by filesystem!\n",
		     __func__, dev->dumname, dev->dumname, dev->dumname);
		pr_info("[dumchar_write] show_stack*************************************\n");
		show_stack(NULL, NULL);
		return -EINVAL;
	}
	...
	if (writesize < 0) {
		pr_info("[dumchar_write] Get writesize fail\n");
		show_stack(NULL, NULL);
		return -EINVAL;
	}
	if ((__u32) *f_pos % (writesize) != 0) {
		pr_info("[dumchar_write] Address Not alignment, write_size=%d byte, address =%lld\n",
		       writesize, *f_pos);
		show_stack(NULL, NULL);
		return -EINVAL;
	}
...
static long dumchar_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (fo->act_filp == (struct file *)0xffffffff) {
		pr_info
		    ("[%s] Forbidded to access %s with dumchar(/dev/%s),the %s partition is managed by filesystem!\n",
		     __func__, dev->dumname, dev->dumname, dev->dumname);
		pr_info("[dumchar_loctl] show_stack*************************************\n");
		show_stack(NULL, NULL);
		return -EINVAL;
	}
	...
	if (dev->type == NAND) {
		...
		if (!strcmp(dev->dumname, "pmt")) {
			...
			case MEMERASE:
				...
				if (!ebuf) {
					pr_info("DumChar: malloc ebuf buffer fail\n");
					return -ENOMEM;
				}
...
loff_t dumchar_llseek(struct file *filp, loff_t off, int whence)
{
	...
	if (fo->act_filp == (struct file *)0xffffffff) {
		pr_info
		    ("[%s] Forbidded to access %s with dumchar(/dev/%s), the %s partition is managed by filesystem!\n",
		     __func__, dev->dumname, dev->dumname, dev->dumname);
		pr_info("[dumchar_llseek] show_stack*************************************\n");
		show_stack(NULL, NULL);
		return -EINVAL;
	}
...
int dumchar_open(struct inode *inode, struct file *filp)
{
	...
	for (i = 0; i < PART_NUM; i++) {
		if (!strcmp(filp->f_path.dentry->d_name.name, dumchar_devices[i].dumname)) {
			...
			fo->index = i;
			found = 1;
			/* pr_info("DumChar: find dev %s index=%d\n",dumchar_devices[i].dumname,i); */
			break;
		}
	}

	if (found == 0) {
		pr_info(" DumChar:ERROR:No Such Dummy Device %s\n ",
	...
	if (!strcmp(dev->dumname, "usrdata") || !strcmp(dev->dumname, "cache")
		...
		pr_info("[dumchar_open][%s] will open %s for %s!\n", current->comm, dev->actname,
		       filp->f_path.dentry->d_name.name);
		...
		if (IS_ERR(fo->act_filp)) {
			...
			pr_info
			    (" DumChar: [%s] open %s failed (%s ).  fo->act_filp=%p!, result=%d\n",
			     ...
			     fo->act_filp, result);
			/* pr_info("[dumchar_open] BUG_ON*************************************\n"); */
			/* BUG_ON(1); */
			/* pr_info("[dumchar_open] ************\n"); */
			goto open_fail2;
		} else {
			if (!(fo->act_filp->f_op)) {
				pr_info
...
int mtd_create_symlink(void)
{
	...
	for (i = 0; i < PART_NUM; i++) {
		if (dumchar_devices[i].actname) {
			...
			pr_info("[mtd_create_symlink]: target=%s, name=%s\n", link_target,
			       link_name);
			sys_symlink(link_target, link_name);
		}
#if 0
	...
	for (i = 0; i < PART_NUM; i++) {
		if ((!strcmp(dumchar_devices[i].dumname, "usrdata"))
			...
			if (sizeof(origin) != ret) {
				pr_info("[mtd_close_read_ahead]read fail!errno=%d\n", ret);
				filp_close(filp, NULL);
				continue;
			}
			ret = filp->f_op->write(filp, &value, sizeof(value), &(filp->f_pos));
			if (sizeof(value) != ret) {
				pr_info("[mtd_close_read_ahead]write fail!errno=%d\n", ret);
				filp_close(filp, NULL);
				continue;
			}
			filp_close(filp, NULL);
			pr_info("[mtd_close_read_ahead]:update succeed!%s from %x to %x\n",
			       target, origin, value);
		}
...
int dumchar_probe(struct platform_device *dev)
{
	...
	pr_info("[Dumchar_probe]*******************Introduction******************\n");
	pr_info("[Dumchar_probe]There are 3 address in eMMC Project: Linear, Logical and Physical Address\n");
	pr_info("[Dumchar_probe]Linear Address: Used in scatter file, uboot, preloader,flash tool etc.\n");
	pr_info("[Dumchar_probe]MBR linear address is fixed in eMMCComo.mk, that is same for all chips\n");
	pr_info("[Dumchar_probe]Logical Address: Used in /proc/dumchar_info, mmcblk0 etc. MBR logical address is 0\n");
	pr_info("[Dumchar_probe]Physical Address: Used in eMMC driver");
	pr_info("MBR Physical Address = MBR Linear Address - (BOOT1 size + BOOT2 Size + RPMB Size)\n");
	pr_info("[Dumchar_probe]define  User_Region_Header (BOOT1 size + BOOT2 Size + RPMB Size)\n");
	pr_info("[Dumchar_probe]*******************Introduction******************\n");
	...
	for (i = 0; i < PART_NUM; i++) {
		...
		if (result) {
			pr_info("DumChar: register char device dumchar fail!\n");
		...
		if (dumchar_devices[i].type == EMMC) {
			...
			case USER:
				...
				break;
			default:
				pr_info("Error! the emmc region is not supportted!\n");
				goto fail_register_chrdev;
			}
			...
			/* } */
			pr_info("[Dumchar] %s start address=%llx size=%llx\n",
	...
	for (l = 0; l < PART_NUM; l++) {
		...
		if (IS_ERR(dumchar_device[l])) {
			...
			pr_info("DumChar: fail in device_create name = %s  minor = %d\n",
...
int dumchar_remove(struct platform_device *dev)
{
	...
	pr_info("DumCharDebug: in dumchar_remove\n");
	for (i = 0; i < PART_NUM; i++) {
...
static int __init dumchar_init(void)
{
	...
	if (!dumchar_devices) {
		result = -ENOMEM;
		pr_info("DumChar: malloc dumchar_dev fail\n");
		goto fail_malloc;
	}
	...
	if (dumchar_proc_entry)
		pr_info("dumchar: register /proc/dumchar_info success\n");
	else {
		pr_info("dumchar: unable to register /proc/dumchar_info\n");
		result = -ENOMEM;
		goto fail_create_proc;
	}
	result = platform_driver_register(&dumchar_driver);
	if (result) {
		pr_info("DUMCHAR: Can't register driver\n");
		goto fail_driver_register;
	}
	result = platform_device_register(&dummychar_device);
	pr_info("[%s]: dummychar_device, retval=%d\n!", __func__, result);
	if (result != 0)
		goto fail_driver_register;

	pr_info("DumChar: init USIF  Done!\n");
