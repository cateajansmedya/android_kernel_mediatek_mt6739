...
#define ISP_SET_BIT(reg, bit)\
	((*(volatile MUINT32*)(reg)) |= (MUINT32)(1 << (bit)))
...
static MUINT32 ISP_DumpDmaDeepDbg(void)
{
	...
	if (g_bDmaERR_p2) {
		LOG_ERR
		("vipi:0x%x,VIPI:0x%x,VIP2I=0x%x,VIP3I=0x%x,MFBO=0x%x,IMG3BO=0x%x,\
			IMG3CO:0x%x,IMG2O:0x%x,IMG3O:0x%x,FEO:0x%x",
...
MBOOL ISP_chkModuleSetting(void)
{
	...
	if (cam_tg1_vf_con & 0x01) {
		...
		MUINT32 afo_d_xsize, afo_d_ysize;
		MUINT32 af_sat_th0, af_sat_th1, af_sat_th2, af_sat_th3;
		MUINT32 TG_W;
		MUINT32 TG_H;
		MUINT32 AF_EN, AFO_D_EN;
		MUINT32 SGG1_EN, SGG5_EN;
		MUINT32 cam_ctrl_en_p1_dma_d;  /*4014*/
		MUINT32 cam_tg_sen_mode, dbl_data_bus;
		MUINT32 tg_w_pxl_e, tg_w_pxl_s;
		MUINT32 tg_h_lin_e, tg_h_lin_s;
		MUINT32 cam_af_con;
		MUINT32 cam_af_th_2;
		MUINT32 cam_af_size, af_image_wd;
		MUINT32 cam_af_vld, af_vld_ystart, af_vld_xstart;
		MUINT32 cam_af_blk_0, af_blk_ysize, af_blk_xsize;
		MUINT32 cam_af_blk_1, af_blk_ynum, af_blk_xnum;
		MUINT32 cam_bmx_crop, bmx_end_x, bmx_str_x;
		MUINT32 cam_rmx_crop, rmx_end_x, rmx_str_x;

		MUINT32 rrz_out_width;
		MUINT32 rrz_out_height;
		MUINT32 rrz_d_out_width;
		MUINT32 rrz_d_out_height;
		MUINT32 scenario;

		MUINT32 cam_rrz_out_img;        /*47A8 */
		MUINT32 cam_rrz_d_out_img;      /*67A8 */
		...
		if ((FLK_SIZE_X % 2 != 0) || (FLK_SIZE_Y % 2 != 0)
			...
			LOG_INF
			("HwRWCtrl:: Flicker Error: CAM_FLK_SIZE.FLK_SIZE_X(%d)\
				and CAM_FLK_SIZE.FLK_SIZE_Y(%d) value can't be 0!!", FLK_SIZE_X, FLK_SIZE_Y);
		}
		if (bmx_enable == MTRUE) {
		...
		if ((((FLK_NUM_X * FLK_SIZE_X) + FLK_OFST_X) > sggmux_h_size) ||
			...
			("HwRWCtrl:: Flicker Error: (CAM_FLK_NUM.FLK_NUM_X.value(%d) *\
				...
				<= sggmux_h_size.value(%d)!!", FLK_NUM_X, FLK_SIZE_X, FLK_OFST_X, sggmux_h_size);
			LOG_INF
			("HwRWCtrl:: Flicker Error: (CAM_FLK_NUM.FLK_NUM_Y.value(%d) *\
				CAM_FLK_SIZE.FLK_SIZE_Y.value(%d)) + CAM_FLK_OFST.FLK_OFST_Y.value(%d)\
				<= sggmux_v_size.value(%d)!!", FLK_NUM_Y, FLK_SIZE_Y, FLK_OFST_Y, sggmux_v_size);
		}
...
AF_EXIT:
	if (rst == MFALSE) {
		...
		if (AAO_InHeight < (AWB_W_VNUM * AWB_W_VPIT + AWB_W_VORG)) {
			...
			("Error HwRWCtrl:: bmx_enable(%d), bmx_width(%d),\
				...
				bmx_enable, bmx_width, bmx_height, grab_width, grab_height);
			LOG_INF
			("Error HwRWCtrl:: input frame height(%d) >= AWB_W_VNUM(%d)\
				* AWB_W_VPIT(%d) + AWB_W_VORG(%d) !!",
		...
		if ((EIS_NUM_VWIN > 8) || (EIS_NUM_HWIN > 4) ||
			...
			("EIS Error, 3. EIS_MF_OFFSET.EIS_RP_VOFST or EIS_RP_HOFST  > 16!!,\
				EIS_RP_VOFST(%d), EIS_RP_HOFST(%d)", EIS_RP_VOFST, EIS_RP_HOFST);
			LOG_INF
		...
		if ((EIS_WIN_HSIZE < (((EIS_NUM_HRP + 1) << 4) + 1)) ||
			...
			/*Error */
			LOG_INF
			("EIS Error, 8. EIS_MB_INTERVAL.EIS_WIN_HSIZE >=\
				(EIS_PREP_ME_CTRL1.EIS_NUM_HRP+1)*16+1!!, EIS_WIN_HSIZE:%d, EIS_NUM_HRP:%d",
				EIS_WIN_HSIZE, EIS_NUM_HRP);
			LOG_INF
			("EIS Error, 9. EIS_MB_INTERVAL.EIS_WIN_VSIZE >=\
				(EIS_PREP_ME_CTRL1.EIS_NUM_VRP+1)*16+1!!, EIS_WIN_VSIZE:%d, EIS_NUM_VRP:%d",
	...
	if (cam_tg2_vf_con & 0x01) {
		...
		if (((AF_WINX_0 % 2) != 0) || ((AF_WINX_1 % 2) != 0) || ((AF_WINX_2 % 2) != 0)
			...
			LOG_INF
			("HwRWCtrl:: AF_D Error: The horizontal window start point\
				and size must be multiples of 2 !!");
			LOG_INF
		...
		if (((AF_FLO_WINX_1 % 2) != 0) || ((AF_FLO_WINX_2 % 2) != 0)
		    ...
		    || ((AF_FLO_HT_3 % 2) != 0)) {
			LOG_INF
			("HwRWCtrl:: AF_D Error: Horizontal start position of\
				floating window must be multiples of 2!!");
			LOG_INF
		...
		if ((AF_WIN_WD > 1020) || (AF_WIN_HT > 1022)) {
			LOG_INF
			("HwRWCtrl:: AF_D Error: Horizontal size of each normal window\
				is limited to 510*2 (AF_WIN_WD(%d))!!", AF_WIN_WD);
			LOG_INF
			("HwRWCtrl:: AF_D Error: Vertical size of each normal window\
				is limited to 511*2 (AF_WIN_HT(%d))!!", AF_WIN_HT);
		}
		...
		if ((AF_WINX_0 < 16) || (AF_LeftOverWidth < 16)) {
			if (AF_WIN_WD < (24 * (1 << AF_DECI))) {
				...
				LOG_INF
				("HwRWCtrl:: AF_D Error: AF_WINX_0(%d), AF_DECI(%d), AF_LeftOverWidth(%d),\
					AF_IMAGE_WD(%d), AF_WIN_WD(%d)!!",
...
static inline void Prepare_Enable_ccf_clock(void)
{
	...
	if (ret) {
		LOG_ERR("cannot get CG_IMG_CAM_SMI clock\n");
	}

	ret = clk_prepare_enable(isp_clk.CG_IMG_CAM_CAM);
	if (ret) {
		LOG_ERR("cannot get CG_IMG_CAM_CAM clock\n");
	}

	ret = clk_prepare_enable(isp_clk.CG_IMG_SEN_TG);
	if (ret) {
		LOG_ERR("cannot get CG_IMG_SEN_TG clock\n");
	}

	ret = clk_prepare_enable(isp_clk.CG_IMG_SEN_CAM);
	if (ret) {
		LOG_ERR("cannot get CG_IMG_SEN_CAM clock\n");
	}

	ret = clk_prepare_enable(isp_clk.CG_IMG_CAM_SV);
	if (ret) {
		LOG_ERR("cannot get CG_IMG_CAM_SV clock\n");
	}
			...
			if (copy_from_user
				...
				/**/                                  if (pstRTBuf->ring_buf[rt_dma].data[i].bufIdx ==
								...
								if (i == ISP_RT_BUF_SIZE) {
									...
									LOG_ERR
									("[rtbc][replace2]error Can't get the idx\
									-(0x%x)/Addr(0x%x) buf\n",
									 deque_buf.data[0].
									 bufIdx,
									 rt_buf_info.
							...
							for (i = 0; i < ISP_RT_BUF_SIZE; i++) {
								if (pstRTBuf->ring_buf[rt_dma].data[i].base_pAddr ==
								 ...
								 if (pstRTBuf->ring_buf[rt_dma].empty_count <
									...
									spin_unlock_irqrestore(&(IspInfo.SpinLockIrq
										  [irqT_Lock]), flags);
									IRQ_LOG_PRINTER(irqT, 0, _LOG_DBG);
									LOG_ERR("[rtbc]error:dma(%d),PA(0x%x),\
									over enque_2", rt_dma,
									pstRTBuf->ring_buf[rt_dma].data[i].base_pAddr);
										return -EFAULT;
									}
								...
								if (1) {
									if (rt_buf_info.bufIdx !=
										...
										LOG_ERR("[rtbc][replace2]error:\
										BufIdx MisMatch. 0x%x/0x%x",
										 rt_buf_info.bufIdx,
										 pstRTBuf->ring_buf[rt_dma].data[i].
											 bufIdx);
						...
						/* make sure rct_inc will be pulled     at the same     vd.     */
							if ((irqT == _IRQ) || (irqT == _IRQ_D)) {
								if ((pstRTBuf->ring_buf[ch_imgo].active == MTRUE)
									&& (MTRUE == pstRTBuf->ring_buf[ch_rrzo].
									active)) {
									if (rt_buf_ctrl.ex_data_ptr != 0) {
...
/* rcnt_sync only work when multi-dma ch enabled. but in order to support multi-enque, these mech. also to be */
									...
									if (MTRUE ==
										...
										(&
										 ...
										 flags);
										LOG_ERR
										("[rtbc]error:dma(%d) are\
										not being activated(%d)",
										 rt_dma,
										 pstRTBuf->
										 ring_buf
										 [rt_dma].
...
#if (ISP_RAW_D_SUPPORT == 1)
							LOG_DBG
							("[rtbc][ENQUE]:dma(%d),PA(0x%x),O(0x%x),ZO(0x%x),O_D(0x%x),\
							ZO_D(0x%x),camsv(0x%x/0x%x)fps(%d/%d/%d/%d)us,rtctrl_%d\n",
			...
			if ((reg_val & 0x01) || (reg_val2 & 0x01) || (camsv_reg_cal[0] & 0x01)
				...
				if (_bFlag == MTRUE) {
					...
					if (IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
						/*LOG_DBG(*/
						IRQ_LOG_KEEPER(irqT,  0, _LOG_DBG,
							       "[rtbc][DEQUE](%d):d(%d)/id(0x%x)/bs(0x%x)/va(0x%llx)/\
								pa(0x%x)/t(%d.%d)/img(%d,%d,%d,%d,%d,%d,%d,%d,%d)/\
								m(0x%x)/fc(%d)/rrz(%d,%d,%d,%d,%d,%d),\
								dmao(%d,%d,%d,%d),lm#(0x%x)",
		...
		if (((P2_EDBUF_MList_LastBufIdx + 1) % _MAX_SUPPORT_P2_PACKAGE_NUM_) ==
			...
			if (P2_EDBUF_MList_LastBufIdx != -1) {
				...
				IRQ_LOG_KEEPER(_CAMSV_D_IRQ, 0, _LOG_DBG,
					       "pD(%d_0x%x) MF/L(%d_%d,%d),(%d,%d), RF/C/L(%d,%d,%d),\
						(%d,%d,%d),dCq(%d)/Bq(%d)\n",
...
static MINT32 ISP_WaitIrq(ISP_WAIT_IRQ_STRUCT * WaitIrq)
{
	...
	if (Timeout == 0) {
		spin_lock_irqsave(&(IspInfo.SpinLockIrq[eIrq]), flags);
		LOG_ERR
		("v1 ERR WaitIrq Timeout Clear(%d), Type(%d), IrqStatus(0x%08X),\
		 WaitStatus(0x%08X), Timeout(%d),user(%d)",
...
static MINT32 ISP_WaitIrq_v3(ISP_WAIT_IRQ_STRUCT * WaitIrq)
{
	...
	if (Timeout == 0) {
		spin_lock_irqsave(&(IspInfo.SpinLockIrq[eIrq]), flags);
		LOG_ERR
		("v3 ERRRR WaitIrq Timeout(%d) Clear(%d), Type(%d), IrqStatus(0x%08X),\
		WaitStatus(0x%08X), Timeout(%d),userKey(%d)\n",
	...
	if (WaitIrq->UserInfo.UserKey > 0) {
		LOG_VRB
		(" [WAITIRQv3]user(%d) mark sec/usec (%d/%d), last irq sec/usec\
			(%d/%d),enterwait(%d/%d),getIRQ(%d/%d)\n",
			WaitIrq->UserInfo.UserKey,
