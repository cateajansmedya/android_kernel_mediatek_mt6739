...
unsigned long g_ulLastTimer_TS_us; /* record previous DVFS applying with timer-based time stamp */
unsigned long g_ulCurVsyncEvent_TS_us; /* record current Vsync Event time stamp */
...
GED_ERROR ged_dvfs_vsync_offset_event_switch(GED_DVFS_VSYNC_OFFSET_SWITCH_CMD eEvent, bool bSwitch)
{
	...
	switch(eEvent)
	{
		...
		case GED_DVFS_VSYNC_OFFSET_GAS_EVENT:
			...
			/* signo if and only if be changed */
		...
		case GED_DVFS_VSYNC_OFFSET_VIDEO_MERGE_MD_EVENT:
			...
			break;
		case GED_DVFS_VSYNC_OFFSET_NETWORK:
			(bSwitch) ? (g_ui32EventStatus |= GED_EVENT_NETWORK) :
				(g_ui32EventStatus &= (~GED_EVENT_NETWORK));
			break;
		case GED_DVFS_VSYNC_OFFSET_DOPT_WIFI_SCAN:
			(bSwitch) ? (g_ui32EventStatus |= GED_EVENT_DOPT_WIFI_SCAN) :
				(g_ui32EventStatus &= (~GED_EVENT_DOPT_WIFI_SCAN));
			break;
		default:
...
GED_ERROR ged_dvfs_um_commit( unsigned long gpu_tar_freq, bool bFallback)
{
	...
	ulTimeDiff = g_ulCurVsyncEvent_TS_us - g_ulLastTimer_TS_us;
	/* check diff between last timer-based if stiring */
	if (ulTimeDiff < 16666) {
		gpu_loading = ulTimeDiff * gpu_loading + GED_DVFS_TIMER_TIMEOUT_US * gpu_pre_loading;
		gpu_loading /= (ulTimeDiff + GED_DVFS_TIMER_TIMEOUT_US);
	}
	...
	{
		ged_dvfs_policy(gpu_loading, &ui32NewFreqID, g_ulCurVsyncEvent_TS_us, 0, 0, true);
	}
	else
	{
		...
		unsigned long t, long phase, unsigned long ul3DFenceDoneTime, bool bRefreshed)
{
	...
	} else if ((phase == GED_DVFS_TIMER_BACKUP) && (ged_gas_query_mode() == GAS_CATEGORY_OTHERS)) {
		/*
		* easy to boost in offscreen cases,
		* and also avoid to apply high power pressure on non-60 FPS Games
		*/
...
void ged_dvfs_sw_vsync_query_data(GED_DVFS_UM_QUERY_PACK* psQueryData)
{
	...
	g_ulCurVsyncEvent_TS_us = psQueryData->usT;
}
