	...
	do { \
		if (clk) { \
			if (clk_prepare_enable(clk)) \
				pr_warn("PVR_K: clk_prepare_enable failed when enabling " #clk); } \
	} while (0)
...
#define MTKCLK_set_parent(clkC, clkP) \
	do { \
		if (clkC && clkP) { \
			...
			pr_warn("PVR_K: clk_set_parent failed when enabling " #clkC, #clkP); } \
	} while (0)
#else
#define MTKCLK_prepare_enable(clk)
...
#endif
static IMG_VOID MTKEnableMfgMtcmos0(void)
{
#ifdef MTK_MFGMTCMOS_AO
...
static IMG_VOID MTKDisableMfgMtcmos0(void)
{
#ifdef MTK_MFGMTCMOS_AO
