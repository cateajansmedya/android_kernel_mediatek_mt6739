...
static const struct dev_pm_ops ITG1010_gyro_pm_ops = {
...
static int ITG1010_gpio_config(void)
{
	...
	if (gyroPltFmDev == NULL) {
		GYRO_ERR("ITG1010_gpio_config NULL : %p, %p\n", gyroPltFmDev, &gyroPltFmDev->dev);
		return -EINVAL;
	}
...
static int ITG1010_i2c_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	strncpy(info->type, ITG1010_DEV_NAME, strlen(ITG1010_DEV_NAME)+1);
	return 0;
}
