...
unsigned int ucDram_Register_Read(unsigned int u4reg_addr)
{
	...
	pu4reg_value = (*(volatile unsigned int *)(DRAMC_AO_CHA_BASE_ADDR + (u4reg_addr))) |
		(*(volatile unsigned int *)(DDRPHY_CHA_BASE_ADDR + (u4reg_addr))) |
		(*(volatile unsigned int *)(DRAMC_NAO_CHA_BASE_ADDR + (u4reg_addr)));
...
unsigned int lpDram_Register_Read(unsigned int Reg_base, unsigned int Offset)
{
	...
	pr_err("lpDram_Register_Read: unsupported Reg_base (%d)\n", Reg_base);
	return 0;
}
...
unsigned int get_dram_data_rate(void)
{
	...
	default:
		pr_err("[DRAMC] MemFreq region is incorrect MEMPLL2_FOUT=%d\n", MEMPLL2_FOUT);
	}
