...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = GC0339_SENSOR_ID,
        //.sensor_id = 0xc8,
	
	.checksum_value = 0xf7375923,
	
	.pre = {
		.pclk = 6000000,				//record different mode's pclk
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x01, 0x32);
	write_cmos_sensor(0x02, 0x0C);
	write_cmos_sensor(0x0F, 0x01);
	write_cmos_sensor(0x6A, 0x77);
		...
		do {
			...
			*sensor_id = read_cmos_sensor(0x00);
			if (*sensor_id == imgsensor_info.sensor_id) {				
				printk("[GC0339]i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
			printk("[GC0339]Read sensor id fail, write_id: 0x%x, sensor_id: 0x%x, GC0339_SENSOR_ID = 0xC8\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
UINT32 GC0339_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
