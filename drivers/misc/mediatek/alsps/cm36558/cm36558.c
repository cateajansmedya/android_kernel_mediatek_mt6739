...
static int CM36558_check_intr(struct i2c_client *client)
{
	...
	pr_notice("CM36558_REG_PS_DATA value value_low = %x, value_reserve = %x\n", databuf[0], databuf[1]);
	...
	pr_notice("CM36558_REG_INT_FLAG value value_low = %x, value_high = %x\n", databuf[0], databuf[1]);
...
static void CM36558_eint_work(struct work_struct *work)
{
	...
	pr_notice("CM36558 int top half time = %lld\n", int_top_time);
...
#ifdef CONFIG_COMPAT
static long cm36558_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long err = 0;
...
#ifdef CONFIG_COMPAT
	.compat_ioctl = cm36558_compat_ioctl,
#endif
};
