...
static void md_cd_dump_ccif_reg(struct ccci_modem *md)
{
	...
	CCCI_MEM_LOG_TAG(md->index, TAG, "md_irq0_mask(%p)=%x\n", md_info->md_ccif_base + 0x20,
		     cldma_read32(md_info->md_ccif_base, 0x20));
	CCCI_MEM_LOG_TAG(md->index, TAG, "md_irq1_mask(%p)=%x\n", md_info->md_ccif_base + 0x24,
		     cldma_read32(md_info->md_ccif_base, 0x24));
