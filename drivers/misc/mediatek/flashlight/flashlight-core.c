...
{
	struct flashlight_operations *pf;
	struct flashlight_operations *pf2;
	struct flashlight_arg fl_arg = {0};
	struct flashlight_arg fl_arg2 = {0};
	struct flashlight_dev_arg fl_dev_arg;
	struct flashlight_dev_arg fl_dev_arg2;
	int ret;
	...
	fl_pr_debug("contrl store.\n");
	if(!strncmp(buf, "on", 2))
	{
		fl_pr_debug("contrl-main on. %s\n", buf);
		fl_arg.level = 1;
		is_main_fl = 1;
	}
	else if(!strncmp(buf, "off", 3))
	{
		fl_pr_debug("contrl-main off. %s\n", buf);
		fl_arg.level = 0;
		is_main_fl = 1;
	}
	else if(!strncmp(buf, "subon", 5))
	{
		fl_pr_debug("contrl-sub subon. %s\n", buf);
		fl_arg2.level = 1;
		is_main_fl = 0;
	}
	else if(!strncmp(buf, "suboff", 6))
	{
		fl_pr_debug("contrl-sub suboff. %s\n", buf);
		fl_arg2.level = 0;
		is_main_fl = 0;
	}
	else
	{
		fl_pr_debug("contrl error: %s\n", buf);
		is_main_fl = 0;
		goto unlock;
	}
	fl_arg.type = 0;	fl_arg.ct = 0;	fl_arg.part = 0;	fl_arg.dur = 0;
	fl_arg2.type = 1;	fl_arg2.ct = 0;	fl_arg2.part = 0;	fl_arg2.dur = 0;

	fl_pr_debug("flashlight_contrl_store. (%d, %d, %d), (%d, %d)\n", fl_arg.type, fl_arg.ct, fl_arg.part, fl_arg.level, fl_arg.dur);
	fl_pr_debug("flashlight_contrl_store. (%d, %d, %d), (%d, %d)\n", fl_arg2.type, fl_arg2.ct, fl_arg2.part, fl_arg2.level, fl_arg2.dur);

	/* call callback function */
	pf = fl_ops[fl_arg.type][fl_arg.ct][fl_arg.part];
	pf2 = fl_ops[fl_arg2.type][fl_arg2.ct][fl_arg2.part];

	if(pf){
		fl_pr_debug("Bob.chen. 3311. \n" );
	}
	else{
		fl_pr_debug("Bob.chen. 3322. \n" );
	}

	if(pf2){
		fl_pr_debug("Bob.chen. 33444. \n" );
	}
	else{
		fl_pr_debug("Bob.chen. 335555. \n" );
	}

	if((fl_arg.level > 0) || (fl_arg2.level > 0))
	{
		if(pf){
			pf->flashlight_set_driver();
		}
		fl_status[fl_arg.type][fl_arg.ct][fl_arg.part] = 1;

		if(pf2){
			pf2->flashlight_set_driver();
		}
		fl_status[fl_arg2.type][fl_arg2.ct][fl_arg2.part] = 1;
	}

	if(pf){
		//ret = fl_set_level(fl_arg.type, fl_arg.ct, fl_arg.part, fl_arg.level); //delete by jsx 20190228 for 8613
		fl_dev_arg.channel = fl_channel[fl_arg.type][fl_arg.ct][fl_arg.part];
		fl_dev_arg.arg = 0;
		ret = pf->flashlight_ioctl(FLASH_IOC_SET_TIME_OUT_TIME_MS, (unsigned long)&fl_dev_arg);
	}

	if(pf2){
		//ret = fl_set_level(fl_arg2.type, fl_arg2.ct, fl_arg2.part, fl_arg2.level);//delete by jsx 20190228 for 8613
		fl_dev_arg2.channel = fl_channel[fl_arg2.type][fl_arg2.ct][fl_arg2.part];
		fl_dev_arg2.arg = 0;
		ret = pf2->flashlight_ioctl(FLASH_IOC_SET_TIME_OUT_TIME_MS, (unsigned long)&fl_dev_arg2);
	}

	if(is_main_fl){
		ret = fl_enable(fl_arg.type, fl_arg.ct, fl_arg.part, fl_arg.level);
	}

	if(!is_main_fl){
		ret = fl_enable(fl_arg2.type, fl_arg2.ct, fl_arg2.part, fl_arg2.level);
	}
	...
	if((fl_arg.level <= 0) && (fl_arg2.level <= 0))
	{
		...
		if(pf2){
			ret = pf2->flashlight_release(NULL);
		}
		fl_status[fl_arg.type][fl_arg.ct][fl_arg.part] = 0;
		fl_status[fl_arg2.type][fl_arg2.ct][fl_arg2.part] = 0;
	}
unlock:
	ret = size;
	return ret;
...
static int flashlight_probe(struct platform_device *dev)
{
	...
	if (device_create_file(flashlight_device, &dev_attr_flashlight_contrl)) {
		fl_pr_err("Failed to create device file(contrl)\n");
