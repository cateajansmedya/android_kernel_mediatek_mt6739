...
static int hct_parse_otg_dts(struct device_node *node, const char *gpio_name, bool gpio_mode)
{
	int gpio_num = 0;
	...
	if (node)
	{
		...
		else //获取GPIO成功
		{
			...
			else //gpio_request 成功
			{
				...
				{
					//hct-drv OTG 使能低有效，其他项目可自行客制化第二个参数的值
...
static int hct_usbc_otg_gpio_num = 0;

int get_usbc_otg_gpio(void)
{
	...
	if (node)
	{
		hct_usbc_otg_gpio_num = hct_parse_otg_dts(node, "gpio-vbus",true);

		if (0 > hct_usbc_otg_gpio_num)
		{
	...
	{
		printk("%s: cannot get the node: 'mediatek,usb_iddig_bi_eint'.\n", __func__);
		return -ENODEV;
	}
