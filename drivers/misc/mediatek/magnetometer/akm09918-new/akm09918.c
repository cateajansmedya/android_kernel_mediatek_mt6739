/*
* Copyright(C)2016 Asahi Kasei Microdevice Corp.
...
//#define AKM_Device_AK8963		//if use AK09918 code to compatible AK8963C, need define this
//#define AKM_Device_AK09911	//if use AK09918 code to compatible AK09911, need define this
//#define AKM_Device_AK09916	//if use AK09918 code to compatible AK09916, need define this

#if AKM09918_DEBUG
#define MAGN_TAG		 "[AKM09918] "
#define MAGN_ERR(fmt, args...)	/*pr_debug*/pr_info(MAGN_TAG fmt, ##args)
#define MAGN_LOG(fmt, args...)	/*pr_debug*/pr_info(MAGN_TAG fmt, ##args)
#define AKMFUNC(fmt, args...)	/*pr_debug*/pr_info(MAGN_TAG fmt, ##args)
#else
#define MAGN_TAG
#define MAGN_ERR(fmt, args...)	do {} while (0)
#define MAGN_LOG(fmt, args...)	do {} while (0)
#define AKMFUNC(fmt, args...)   do {} while (0)
#endif
...
static long AKECS_SetMode_SngMeasure(void)
{
	...
	#else
	buffer[0] = AK09918_REG_CNTL2;
	buffer[1] = AK09918_MODE_SNG_MEASURE;
	#endif
...
static long AKECS_SetMode_SelfTest(void)
{
	...
	#else
	buffer[0] = AK09918_REG_CNTL2;
	buffer[1] = AK09918_MODE_SELF_TEST;
	#endif
	/* Set data */
	return AKI2C_TxData(buffer, 2);
}
...
static int AKECS_SetMode_PowerDown(void)
{
	...
	#else
	buffer[0] = AK09918_REG_CNTL2;
	buffer[1] = AK09918_MODE_POWERDOWN;
	#endif
	/* Set data */
	return AKI2C_TxData(buffer, 2);
}
...
static int AKECS_GetRawData(char *rbuf, int size)
{
	...
	if (AKECS_GetData(strbuf, SENSOR_DATA_SIZE) < 0)
		MAGN_LOG("%s:%d Error.\n", __func__, __LINE__);
...
int FST_AK09918(void)
{
	...
	// TEST
	TEST_DATA(TLIMIT_NO_RST_WIA1_09918,   TLIMIT_TN_RST_WIA1_09918,   (int)i2cData[0],  TLIMIT_LO_RST_WIA1_09918,   TLIMIT_HI_RST_WIA1_09918,   &pf_total);
	TEST_DATA(TLIMIT_NO_RST_WIA2_09918,   TLIMIT_TN_RST_WIA2_09918,   (int)i2cData[1],  TLIMIT_LO_RST_WIA2_09918,   TLIMIT_HI_RST_WIA2_09918,   &pf_total);

	// Set to PowerDown mode
	if (AKECS_SetMode(AK09918_MODE_POWERDOWN) < 0) {
		MAGN_LOG("%s:%d Error.\n", __FUNCTION__, __LINE__);
		return 0;
	}
	...
	// Set to SNG measurement pattern (Set CNTL register)
	if (AKECS_SetMode(AK09918_MODE_SNG_MEASURE) < 0) {
		MAGN_LOG("%s:%d Error.\n", __FUNCTION__, __LINE__);
		return 0;
	}
	...
	// TEST
	//i2cData[0] &= 0x7F;
	TEST_DATA(TLIMIT_NO_SNG_ST1_09918,  TLIMIT_TN_SNG_ST1_09918,  (int)i2cData[0], TLIMIT_LO_SNG_ST1_09918,  TLIMIT_HI_SNG_ST1_09918,  &pf_total);

	// TEST
	TEST_DATA(TLIMIT_NO_SNG_HX_09918,   TLIMIT_TN_SNG_HX_09918,   hdata[0],          TLIMIT_LO_SNG_HX_09918,   TLIMIT_HI_SNG_HX_09918,   &pf_total);
	TEST_DATA(TLIMIT_NO_SNG_HY_09918,   TLIMIT_TN_SNG_HY_09918,   hdata[1],          TLIMIT_LO_SNG_HY_09918,   TLIMIT_HI_SNG_HY_09918,   &pf_total);
	TEST_DATA(TLIMIT_NO_SNG_HZ_09918,   TLIMIT_TN_SNG_HZ_09918,   hdata[2],          TLIMIT_LO_SNG_HZ_09918,   TLIMIT_HI_SNG_HZ_09918,   &pf_total);
	TEST_DATA(TLIMIT_NO_SNG_ST2_09918,  TLIMIT_TN_SNG_ST2_09918,  (int)i2cData[8], TLIMIT_LO_SNG_ST2_09918 & TLIMIT_ST2_MASK_09918,  TLIMIT_HI_SNG_ST2_09918,  &pf_total);

	// Set to Self-test mode (Set CNTL register)
	if (AKECS_SetMode(AK09918_MODE_SELF_TEST) < 0) {
		MAGN_LOG("%s:%d Error.\n", __FUNCTION__, __LINE__);
		return 0;
	}
	...
	// TEST
	i2cData[0] &= 0x7F;
	TEST_DATA(TLIMIT_NO_SLF_ST1_09918, TLIMIT_TN_SLF_ST1_09918, (int)i2cData[0], TLIMIT_LO_SLF_ST1_09918, TLIMIT_HI_SLF_ST1_09918, &pf_total);
	...
	// TEST
	TEST_DATA(
			  TLIMIT_NO_SLF_RVHX_09918,
			  TLIMIT_TN_SLF_RVHX_09918,
			  hdata[0],
			  TLIMIT_LO_SLF_RVHX_09918,
			  TLIMIT_HI_SLF_RVHX_09918,
			  &pf_total
			  );

	TEST_DATA(
			  TLIMIT_NO_SLF_RVHY_09918,
			  TLIMIT_TN_SLF_RVHY_09918,
			  hdata[1],
			  TLIMIT_LO_SLF_RVHY_09918,
			  TLIMIT_HI_SLF_RVHY_09918,
			  &pf_total
			  );

	TEST_DATA(
			  TLIMIT_NO_SLF_RVHZ_09918,
			  TLIMIT_TN_SLF_RVHZ_09918,
			  hdata[2],
			  TLIMIT_LO_SLF_RVHZ_09918,
			  TLIMIT_HI_SLF_RVHZ_09918,
			  &pf_total
			  );

		TEST_DATA(
			TLIMIT_NO_SLF_ST2_09918,
			TLIMIT_TN_SLF_ST2_09918,
			(int)i2cData[8] & TLIMIT_ST2_MASK_09918,
			TLIMIT_LO_SLF_ST2_09918,
			TLIMIT_HI_SLF_ST2_09918,
			&pf_total
			);
...
static ssize_t show_sensordata_value(struct device_driver *ddri, char *buf)
{
	...
	if (atomic_read(&open_flag) == 0) {
		AKECS_SetMode_SngMeasure();
		mdelay(10);
		if (AKECS_GetData(sensordata, SENSOR_DATA_SIZE) < 0)
			MAGN_LOG("%s:%d Error.\n", __func__, __LINE__);
	} else {
		mutex_lock(&sense_data_mutex);
...
static ssize_t store_layout_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (ret == 0) {
		...
		if (!hwmsen_get_convert(layout, &data->cvt))
			...
			if (!hwmsen_get_convert(0, &data->cvt))
				MAG_ERR("HWMSEN_GET_CONVERT function error!\r\n");
		}
	} else
...
static ssize_t show_trace_value(struct device_driver *ddri, char *buf)
{
	...
	if (NULL == obj) {
		MAG_ERR("akm09918_i2c_data is null!!\n");
		return 0;
	}
...
{
	int idx, err = 0;
	int num = (int)(sizeof(akm09918_attr_list)/sizeof(akm09918_attr_list[0]));
...
/*----------------------------------------------------------------------------*/
static int akm09918_open(struct inode *inode, struct file *file)
{
	struct akm09918_i2c_data *obj = i2c_get_clientdata(this_client);
	int ret = -1;

	if (atomic_read(&obj->trace) & AMK_CTR_DEBUG)
		MAGN_LOG("Open device node:akm09918\n");
...
/*----------------------------------------------------------------------------*/
static int akm09918_release(struct inode *inode, struct file *file)
{
	struct akm09918_i2c_data *obj = i2c_get_clientdata(this_client);

	atomic_dec(&dev_open_count);
	if (atomic_read(&obj->trace) & AMK_CTR_DEBUG)
		MAGN_LOG("Release device node:akm09918\n");
...
static long akm09918_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	char buff[AKM09918_BUFSIZE];		/* for chip information */
  ...
  /* MAG_ERR("akm09918 cmd:0x%x\n", cmd); */
	...
	case ECS_IOCTL_GET_DELAY_09918:
	...
	case ECS_IOCTL_GET_LAYOUT_09918:
	...
	case MSENSOR_IOCTL_READ_CHIPINFO:
		...
		akm09918_ReadChipInfo(buff, AKM09918_BUFSIZE);
	...
	case MSENSOR_IOCTL_READ_SENSORDATA:
		...
		AKECS_GetRawData(buff, AKM09918_BUFSIZE);
...
#ifdef CONFIG_COMPAT
static long akm09918_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long ret;
	...
	case COMPAT_ECS_IOCTL_SET_YPR_09911:
		ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_SET_YPR_09918,
				(unsigned long)(arg32));
		if (ret) {
			MAGN_LOG("ECS_IOCTL_SET_YPR_09918 unlocked_ioctl failed.");
			return ret;
		}
	...
	case COMPAT_ECS_IOCTL_GET_DELAY_09911:
		ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_GET_DELAY_09918,
				(unsigned long)(arg32));
		if (ret) {
			MAGN_LOG("ECS_IOCTL_GET_DELAY_09918 unlocked_ioctl failed.");
			return ret;
		}
	...
	case COMPAT_ECS_IOCTL_GET_LAYOUT_09911:
		ret = file->f_op->unlocked_ioctl(file, ECS_IOCTL_GET_LAYOUT_09918,
				(unsigned long)arg32);
		if (ret) {
			MAGN_LOG("ECS_IOCTL_GET_LAYOUT_09918 unlocked_ioctl failed.");
			return ret;
		}
...
/*----------------------------------------------------------------------------*/
static const struct file_operations akm09918_fops = {
	.owner = THIS_MODULE,
	.open = akm09918_open,
	.release = akm09918_release,
	/* .unlocked_ioctl = akm09918_ioctl, */
	.unlocked_ioctl = akm09918_unlocked_ioctl,
#ifdef CONFIG_COMPAT
	.compat_ioctl = akm09918_compat_ioctl,
#endif
};
/*----------------------------------------------------------------------------*/
static struct miscdevice akm09918_device = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "msensor",
	.fops = &akm09918_fops,
};
/*----------------------------------------------------------------------------*/
int akm09918_operate(void *self, uint32_t command, void *buff_in, int size_in,
...
/*----------------------------------------------------------------------------*/
int akm09918_orientation_operate(void *self, uint32_t command, void *buff_in, int size_in,
	void *buff_out, int size_out, int *actualout)
{
	int err = 0;
	int value;
...
#if DEBUG
	if (atomic_read(&data->trace) & AMK_FUN_DEBUG)
		AKMFUNC("akm09918_orientation_operate");
#endif
...
/*----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------*/
int akm09918_gyroscope_operate(void *self, uint32_t command, void *buff_in, int size_in,
	void *buff_out, int size_out, int *actualout)
{
	int err = 0;
	int value;
...
#if DEBUG
	if (atomic_read(&data->trace) & AMK_FUN_DEBUG)
		AKMFUNC("akm09918_gyroscope_operate");
#endif
...
/*----------------------------------------------------------------------------*/
int akm09918_rotation_vector_operate(void *self, uint32_t command, void *buff_in, int size_in,
	void *buff_out, int size_out, int *actualout)
{
	int err = 0;
	int value;
...
#if DEBUG
	if (atomic_read(&data->trace) & AMK_FUN_DEBUG)
		AKMFUNC("akm09918_rotation_vector_operate");
#endif
...
/*----------------------------------------------------------------------------*/
int akm09918_gravity_operate(void *self, uint32_t command, void *buff_in, int size_in,
	void *buff_out, int size_out, int *actualout)
{
	int err = 0;
	int value;
...
#if DEBUG
	if (atomic_read(&data->trace) & AMK_FUN_DEBUG)
		AKMFUNC("akm09918_gravity_operate");
#endif
...
/*----------------------------------------------------------------------------*/
int akm09918_linear_accelration_operate(void *self, uint32_t command, void *buff_in, int size_in,
	void *buff_out, int size_out, int *actualout)
{
	int err = 0;
	int value;
...
#if DEBUG
	if (atomic_read(&data->trace) & AMK_FUN_DEBUG)
		AKMFUNC("akm09918_linear_accelration_operate");
#endif
...
static int akm09918_i2c_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	strncpy(info->type, AKM09918_DEV_NAME, sizeof(info->type));
	return 0;
}

static int akm09918_m_enable(int en)
{
	int value = 0;
	int err = 0;
	...
	if (value == 1) {
		...
		err = AKECS_SetMode(AK09918_MODE_SNG_MEASURE);
		if (err < 0) {
		...
		if (atomic_read(&o_flag) == 0) {
			atomic_set(&open_flag, 0);
			err = AKECS_SetMode(AK09918_MODE_POWERDOWN);
			if (err < 0) {
				MAG_ERR("%s:AKECS_SetMode Error.\n", __func__);
				return err;
			}
...
static int akm09918_m_set_delay(u64 ns)
{
	int value = 0;
...
static int akm09918_m_open_report_data(int open)
{
	return 0;
}

static int akm09918_m_get_data(int *x , int *y, int *z, int *status)
{
	mutex_lock(&sensor_data_mutex);
...
static int akm09918_o_enable(int en)
{
	int value = 0;
	int err = 0;
...
static int akm09918_o_set_delay(u64 ns)
{
	int value = 0;
...
static int akm09918_o_open_report_data(int open)
{
	return 0;
}

static int akm09918_o_get_data(int *x , int *y, int *z, int *status)
{
	mutex_lock(&sensor_data_mutex);
...
static int akm09918_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	data->client = client;
	data->client->addr = 0x0c;	//forced change i2c slave addr
	...
	if (err < 0) {
		MAG_ERR("AKM09918 akm09918_probe: check device connect error\n");
	...
	err = misc_register(&akm09918_device);
	if (err) {
		MAG_ERR("akm09918_device register failed\n");
		goto exit_misc_device_register_failed;
	}

	ctl.is_use_common_factory = false;
	ctl.m_enable = akm09918_m_enable;
	ctl.m_set_delay  = akm09918_m_set_delay;
	ctl.m_open_report_data = akm09918_m_open_report_data;
	ctl.o_enable = akm09918_o_enable;
	ctl.o_set_delay  = akm09918_o_set_delay;
	ctl.o_open_report_data = akm09918_o_open_report_data;
	...
	mag_data.get_data_o = akm09918_o_get_data;
	mag_data.get_data_m = akm09918_m_get_data;
...
static int akm09918_i2c_remove(struct i2c_client *client)
{
	...
	if (err)
		MAG_ERR("akm09918_delete_attr fail: %d\n", err);

	this_client = NULL;
	i2c_unregister_device(client);
	kfree(i2c_get_clientdata(client));
	misc_deregister(&akm09918_device);
	return 0;
}
