...
unsigned int idle_condition_mask[NR_TYPES][NR_GRPS] = {
	...
	[IDLE_TYPE_DP] = {
		...
		0x03000000,	/* MFG, TOPCLK CLK_CFG_1 mfg_sel */
...
static void get_all_clock_state(u32 clks[NR_GRPS])
{
	...
	/* Patch: Check Topclk CLK_CFG_1 mfg_sel = 0 (clk26m) instead */
	if (sys_is_on(SYS_MFG))
		clks[CG_MFG] = idle_readl(CLK_CFG(1));
		/* clks[CG_MFG] = ~idle_readl(SPM_MFG_PWR_CON); */    /* MTCMOS MFG */

	if (sys_is_on(SYS_VCODEC))
		clks[CG_VCODEC] = ~idle_readl(SPM_VDE_PWR_CON); /* MTCMOS VCODEC */
}
...
void __init iomap_init(void)
{
	...
	get_base_from_node("mediatek,topckgen", &cg_topck_base, 0);
}
