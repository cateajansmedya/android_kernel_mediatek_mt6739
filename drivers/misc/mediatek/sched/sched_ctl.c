...
/* switchable aggressive load-balance */
#define AGGRESSIVE_MIGRATION_COST 33000
static unsigned int tmp_migration_cost;
#endif

static ssize_t store_lb_enhance(struct kobject *kobj,
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%iu", &val) != 0) {
		if (val >= 0) {
			if (!val) {
				/* turn it off */
				sysctl_sched_mtk_lb_enhance = 0;

				/* restore  migration cost if no one change it. */
				if (sysctl_sched_migration_cost == AGGRESSIVE_MIGRATION_COST)
					sysctl_sched_migration_cost = tmp_migration_cost;
			} else {
				/* turn it on */
				sysctl_sched_mtk_lb_enhance = 1;
				tmp_migration_cost = sysctl_sched_migration_cost;
				sysctl_sched_migration_cost = AGGRESSIVE_MIGRATION_COST;
			}
		}
	}
	return count;
...
static ssize_t show_lb_enhance(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	unsigned int len = 0;
	unsigned int max_len = 4096;

	len +=  snprintf(buf, max_len, "sysctl_sched_migration_cost=%d\n",
					sysctl_sched_migration_cost);
	len +=  snprintf(buf+len, max_len - len, "sysctl_sched_mtk_lb_enhance=%d\n",
					sysctl_sched_mtk_lb_enhance);
...
/* switchable aggressive load-balance */
static struct kobj_attribute sched_mtk_lb_enhance_attr =
__ATTR(lb_enhance_enable, S_IWUSR | S_IRUSR, show_lb_enhance, store_lb_enhance);

static struct attribute *sched_attrs[] = {
	&sched_info_attr.attr,
	&sched_load_thresh_attr.attr,
	&sched_enable_attr.attr,
	&sched_mtk_lb_enhance_attr.attr,
	NULL,
};
