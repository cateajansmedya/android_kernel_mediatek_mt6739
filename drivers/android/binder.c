...
#ifdef BINDER_MONITOR
	strncpy(binder_context_mgr_node->name, "servicemanager", MAX_SERVICE_NAME_LEN);
...
static int binder_node_release(struct binder_node *node, int refs)
{
	...
	if (node->proc && node->proc->tsk) {
		strncpy(dead_pname, node->proc->tsk->comm, TASK_COMM_LEN);
		dead_pname[TASK_COMM_LEN - 1] = '\0';
	}
#endif
